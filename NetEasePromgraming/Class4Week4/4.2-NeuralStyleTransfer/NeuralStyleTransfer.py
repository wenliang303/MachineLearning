...
def _weights(layer, expected_layer_name):
    ...
    print(type(expected_layer_name))
...
def compute_content_cost(a_C, a_G):
    ...
    J_content = tf.divide(tf.reduce_sum(tf.square(a_C_unrolled - a_G_unrolled)), 4 * n_H * n_W * n_C)
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = tf.divide(tf.reduce_sum(tf.square(GS - GG)), (2 * n_C * n_H * n_W) ** 2)
...
def SolvingTheOptimizationProblem():
...
test_gram_matrix()
test_compute_layer_style_cost()
test_total_cost()
SolvingTheOptimizationProblem()

...
def get_processed_data():
    X_orig,y_orig = get_original_data()
    X = np.insert(X_orig, 0, np.ones(X_orig.shape[0]), axis=1)#增加全部为1的一列
    encoder = OneHotEncoder(sparse=False)
    y_onehot = encoder.fit_transform(y_orig)
...
def gradient(theta, X, y):
    ...
    for i in range(m):
        ...
        d2i = np.multiply(np.dot(t2.T , d3i), sigmoid_gradient(z2i))  # (1, 26)

        # careful with np vector transpose
        delta2 += np.dot(np.matrix(d3i).T , np.matrix(a2i))  # (1, 10).T @ (1, 26) -> (10, 26)
        delta1 += np.dot(np.matrix(d2i[1:]).T , np.matrix(a1i))  # (1, 25).T @ (1, 401) -> (25, 401)
...
def expand_array(arr):
    ...
    return np.array(np.dot(np.matrix(np.ones(arr.shape[0])).T,  np.matrix(arr)))
...
def test_plot_100_image():
    X,_=get_original_data()
    plot_100_image(X)
    plt.show()

def test_cost():

    X,y,theta = get_processed_data()

    print (cost(theta,X,y))
    print (regularized_cost(theta, X, y))

def test_grident_checking():
    X,y,theta = get_processed_data()
...
def test_training():
    X,y,theta = get_processed_data()
    _,y_orig = get_original_data()
    res = nn_training(X, y)#慢
    print (res)

    final_theta = res.x

    print (show_accuracy(theta,X,y_orig))
